...
const baseClasses = ['cursor-pointer', 'whitespace-nowrap', 'rounded-full', 'border', 'border-transparent', 'shadow-sm', 'hover:bg-opacity-90', 'capitalize', 'font-semibold', 'inline-flex', 'flex-shrink-0', 'items-center', 'justify-center', 'transition-colors', 'ease-in-out', 'duration-500'];

const ButtonVariants = cva('h-fit text-white uppercase transition-colors duration-150', {
  variants: {
    intent: {
...
const Button = forwardRef<typeof BaseButton, ButtonProps>(
  (
    ...
    const elementProps = {
      ...(as ? { target: isExternal ? "_blank" : undefined, rel: isExternal ? "noopener noreferrer" : undefined } : {}),
