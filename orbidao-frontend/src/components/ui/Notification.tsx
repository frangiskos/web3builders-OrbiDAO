...
import { NotificationActionType } from '@/models/types';

export const NotificationList = () => {
  const { notifications, setNotificationStore } = useNotificationStore((s) => s);
...
const Notification = ({ type, message, description, txid, onHide }: NotificationActionType) => {
  const { connection } = useConnection();

  // TODO: we don't have access to the network or endpoint here..
  ...
  return (
    <div
      className={`max-w-sm w-full bg-slate-600 shadow-lg rounded-md mt-2 pointer-events-auto ring-1 ring-black ring-opacity-5 p-2 mx-4 mb-4 overflow-hidden`}
    >
      <div className={`p-4`}>
